...
| **12** | **[Poster build sheet — v2](12-poster-build-sheet-v2.md)** | Not an investigation — the translation from written panels to a physical 3ft × 4ft board: what goes in each cell, at what point size, which figure it carries, and what has to be true before it can be printed. **v2 is the one to work from.** [v1](12-poster-build-sheet-v1.md) is kept because it is the version the first eight cells were laid out against; v2 cuts two cells that needed none of the factory and restores the two panels about other people using it. Numbered 12 because report 11 was taken; "report 11" in prose always means the dev-split one. |
